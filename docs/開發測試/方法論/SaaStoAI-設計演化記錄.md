...
**人類要的是一個按鈕 → 專業水準的結果。但結果不會百分之百完美。**

```
階段 1（背後）：預校準
  誰：創辦人 / 領域專家
  做什麼：建 Pack，把品質調到「第 10 次」的水準
  使用者感知：不知道這段存在
...
**工作檯不是編輯器，不是取代 Word/Excel。**

錯誤的設計預設：
> 人類會離開 Word/Excel → 來我們的工作檯編輯/迭代 → 養成習慣
> （不可能。沒有人會放棄自己熟悉的工具。）

正確的邏輯：
- 工作檯存在是因為**一鍵生成的結果不完美，使用者會被老闆罵**
- 工作檯讓使用者**有信心交出去**（審查 ≠ 編輯）
- 專業人士可以在工作檯上**干預生成條件**（不是改成果內容，那回 Word 改就好）
- 專業人士改的是「為什麼這樣生不對、應該注意什麼、什麼情況要怎麼處理」
- 這些腦中的經驗，透過工作檯的操作，被系統轉成 OverlayPatch / skill
- **工作檯是經驗萃取器，不是內容編輯器**

### 使用者在工作檯上的動作 = 自然校準

使用者不覺得自己在「訓練 AI」，他覺得自己在「檢查成果能不能交」。
但每一個接受/拒絕/標記的動作，剛好就是校準訊號。
**這就是 v0.3 說的「自然動作 = 校準」。**
...
| 專業人士 | 干預生成條件、編輯校準規則 | 深度校準（modify/boundary），可轉成 skill |

專業人士的經驗是最有價值的 — 它可以被轉成 AI 能懂的 skill，讓一鍵生成的品質從「還行」變成「專業水準」。
...
預校準（Pack）→ 一鍵生成 → 結果
                   ...
                   工作檯審查（使用者/專業人士）
                               ↓
              CalibrationSignal → OverlayPatch
                               ↓
                      下次一鍵品質更好
                               ↓
                    校準成本隨使用遞減
...
2. **產品是三段式：預校準 → 一鍵生成 → 工作檯審查。不要設計成只有其中一段**
3. **工作檯是經驗萃取器，不是內容編輯器。** 不要設計成取代 Word/Excel 的編輯工具。使用者在工作檯上做的是「確認能不能交」和「告訴系統什麼是對的」，不是「在這裡完成工作」
4. **人類討厭迭代。** 一鍵生成是最低要求。工作檯存在是因為結果不完美、使用者需要有信心交出去
5. **專業人士在工作檯上干預的是生成條件（校準規則），不是成果內容。** 這些經驗可以被轉成 AI 能懂的 skill
6. 不同文件用不同工程語言表達同一件事，不要當成不同方向
7. 已經有兩份成熟的工程文件（architecture-plan + v0.3），不要再從零設計
8. 缺的是「橋接文件」— 把校準閉環放到執行平台上
9. 任何新提案必須回答：它解決了上面哪個缺口？它跟已有的兩份文件怎麼接？
10. 不要再產出第三份獨立架構文件
11. saastoai-ui（Astro + React islands）是最可能的產品載體，不是 vm0

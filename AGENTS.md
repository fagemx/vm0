...
### Find Examples First, Don't Guess

This is a mature codebase with rich examples. Before implementing anything:

1. **Search for similar implementations** - Most features have existing patterns to follow
2. **Check project docs** - `docs/`, `.claude/skills/`, `CLAUDE.md`
3. **Follow project conventions** - Naming, file structure, parameter formats
4. **External APIs must be verified** - Never assume parameter formats; find existing usage or explicitly state "needs verification"

**Core principle: What you need to do has likely been done before. Search first, don't guess.**

### Search Strategy

When exploring the codebase:

- **Use broad queries first** - "authentication flow" not "checkAuth"
- **Split compound questions** - Search separately, don't combine
- **Search multiple times with different wording** - First results often miss key details
- **Keep searching until confident** - Don't stop at the first seemingly relevant result

### Staged Thinking

Process tasks through these internal stages:

| Stage | Goal | Key Actions |
|-------|------|-------------|
| **Understand** | Gather information | Search with different keywords; trace symbols to definitions and usages |
| **Verify** | Validate understanding | Ensure conclusions have code evidence (file paths, line numbers) |
| **Plan** | Define steps | List concrete steps referencing specific files; consider side effects |
| **Execute** | Implement | Execute step by step; verify after each step |
| **Summarize** | Confirm completion | Ensure requirements met; suggest follow-ups if any |

### Hard Gates

**Gate 1: Read-before-Write**
Before editing any file, you must have:
- Read the relevant section (at least 30 lines around the target), OR
- Grep hit with sufficient context (-C 3 minimum)

**Gate 2: Baseline / Regression**
- Run linter/type check BEFORE first edit to establish baseline
- Run same checks AFTER edits
- New errors = assume you caused them unless baseline proves otherwise

**Gate 3: Major Change Definition**
The following require user confirmation before execution:
- Schema changes (DB, storage, file formats)
- Cross-module contract changes (types, API, props)
- Changes affecting 3+ files across layers (UI + state + service)
- Dependency changes, build/test config, routing/security changes

**Gate 4: Evidence Ledger**
Every key conclusion must have supporting evidence:
- File path
- Content snippet or line range
- What conclusion this evidence supports

Never claim "confirmed" or "understood" without evidence.

### Execution Readiness Checklist

Before modifying code, ensure you know:
- [ ] Which files to modify (exact paths)
- [ ] Which functions to modify (exact names)
- [ ] What side effects these changes will have
- [ ] How to verify the changes work

If any item is uncertain, keep searching until certain.